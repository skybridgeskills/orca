...
| Steward                    | An org member assigned to an achievement (`Achievement.json.stewards`) who can approve its claims directly — an additive overlay on any review rule, like a per-achievement review admin. See ADR 2026-06-09-steward-review-and-messaging.                                                                                               |
| Message                    | A log row recording a user-directed message (e.g. a steward review notification). Drives notification throttling (3-day) and retention (30-day GC).                                                                                                                                                                                      |
| rubric / ResultDescription | An achievement's review criteria (`Achievement.json.resultDescriptions`): each criterion has a name and ordered allowed values. Reviewers record a `Result` (chosen value) per criterion on their endorsement; editing a criterion re-mints its id, marking prior reviews "not current". See ADR 2026-06-09-achievement-rubrics-results. |
