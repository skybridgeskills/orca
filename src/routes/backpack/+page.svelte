<script lang="ts">
  ...
  let { page: currentPageNum, pageSize } = $state(
    calculatePageAndSize($page.url)
  );
  let currentPageData = $derived(
    $backpackClaims.slice(
      (currentPageNum - 1) * pageSize,
      currentPageNum * pageSize
    )
  );

  let currentShareIntent:
    | (AchievementClaim & { achievement: Achievement })
    | null = $state(null);
  ...
  const handleShare = (
    claim: AchievementClaim & { achievement: Achievement }
  ) => {
    if (navigator.share) {
      navigator.share({
  ...
  const handleCopyToClipboard = (
    ...
    navigator.clipboard.writeText(
      `${PUBLIC_HTTP_PROTOCOL}://${data.org.domain}/ob2/a/${claim.id}`
    );
  };
...
>
  <p
    class="max-w-2xl mb-4 lg:mb-8 text-gray-500 text-sm md:text-md dark:text-gray-400"
  >
    {m.share_description()}
  </p>
  ...
  {#if currentShareIntent && currentShareIntent?.achievement}
    <a
      href={linkedInShareUrl({
        ...currentShareIntent,
        achievement: {
          ...currentShareIntent.achievement,
