<script lang="ts">
  ...
  const levelByStatus: { [key: string]: App.NotificationLevel } = {
    UNACCEPTED: "warning",
    REJECTED: "warning",
    ACCEPTED: "info",
  };
...
<div class="max-w-2xl mt-2">
    ...
    {#snippet actions()}
      <span>
        {#if data.hasProvidedEndorsement}
          <a href={`${data.claim.id}/endorse`}
            ><Button text={m.endorsement_updateCTA()} /></a
          >
        {:else if data.session?.user?.id != data.claim.userId}
          <a href={`${data.claim.id}/endorse`}
            ><Button text={m.endorsement_addYoursCTA()} /></a
          >
        {/if}
      </span>
    {/snippet}
  ...
  {:else if data.achievement.achievementConfig?.reviewsRequired && data.achievement.achievementConfig?.reviewRequires}
    <p class="max-w-2xl my-4 text-sm text-gray-500 dark:text-gray-400">
      ...
      <a
        href="/achievements/{data.achievement.achievementConfig.reviewRequires
          .id}"
