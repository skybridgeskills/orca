...
>
      ...
      {#each $notifications as n (n.id)}
        ...
        >
          {#each n.actions as action}
            <a href={action.href} class="underline hover:no-underline font-bold"
              >{action.label}</a
            >
          {/each}
        </Alert>
  ...
  <footer
    class="app-footer p-4 bg-white shadow md:px-6 md:py-8 dark:bg-gray-800"
  >
    <div class="max-w-7xl mx-auto">
      <div class="sm:flex sm:items-end sm:justify-between">
        ...
        >
          ...
          <li>
            <a href="/terms" class="mr-4 hover:underline md:mr-6"
              >{m.termsOfService()}</a
            >
          </li>
          <li>
