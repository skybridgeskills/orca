<script lang="ts">
  ...
  let config: (AchievementConfig & App.AchievementConfigWithJson) | null =
    $state(null);
  let claim: AchievementClaim | undefined = $state();
  let category: AchievementCategory | undefined = $state();
  let userHoldsRequiredAchievement = $state(false);
  let inviteCapability = $state(false);
  let reviewRequires: Achievement | undefined = $state();
  ...
  run(() => {
    config = data.achievement.achievementConfig as
      ...
      | null;
    claim = data.relatedClaims.find(
      (c) => data.achievement.id == c.achievementId
    );
    userHoldsRequiredAchievement =
      data.relatedClaims.filter(
        (c) => c.achievementId == config?.claimRequiresId
      ).length > 0;
    inviteCapability =
    ...
    reviewRequires = data.relatedAchievements
      .filter(
        (c) => data.achievement.achievementConfig?.reviewRequiresId == c.id
      )
      .find(() => true);
...
<div class="max-w-2xl flex justify-between items-center mb-4">
  <h1
    class="inline-flex mt-1 mr-3 text-xl sm:text-2xl text-gray-800 dark:text-white"
  >
...
<div class="mt-4 max-w-2xl">
  <Heading title={m.claimConfiguration_heading()} level="h3">
    ...
    {#if config?.claimable && config?.claimRequiresId}
      ...
      {@const claimRequires = $achievements.find(
        (a) => config?.claimRequiresId == a.id
      )}
      {#if claimRequires}
    ...
    {#if inviteRequires}
      ...
      <a
        href={`/achievements/${inviteRequires?.id}`}
