...
{#if data.highlightedAchievements.length}
  <h2 class="text-xl sm:text-2xl mb-3 dark:text-white">
    {m.mostAwardedAchievements()}
  </h2>
    ...
    {#each data.highlightedAchievements as achievement}
      <Card maxWidth="">
        ...
        <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">
          {m.achievement_awardCountSummary({
            count: achievement._count.achievementClaims,
          })}
        </p>
  ...
  <p class="my-4 text-sm text-gray-500 dark:text-gray-400 text-center">
    {m.seeCTA()}
    <a href="/achievements" class="underline hover:no-underline"
      >{m.achievements_all()}</a
    >.
  </p>
{:else}
